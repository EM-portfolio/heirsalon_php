...
function makeReservationButton(){
  ...
  const scheduleElement = document.querySelector('.fc-event-time');
    if(activeElement.length >= 2 && scheduleElement.classList.contains('fc-event-time')){
      ...
      var reservationButton1 = document.createElement('button');
      reservationButton1.classList.add('reservationButton1');
      reservationButton1.textContent = "上記の内容で予約する";
      reservationButton1.id = 'modalbtn';
      document.getElementById('menulist').appendChild(reservationButton1);
...
document.getElementById('menulist').addEventListener('click', function (event) {
  ...
  updateReservationDisplay(document.querySelector('#start')?.getAttribute('data-select-starttime'));
  // 日本時間（JST）に変換する関数
  ...
  if (event.target.classList.contains('reservationButton1')) {
    ...
    window.onclick = function(event) {
        ...
        // 内容を表示する
        document.getElementById('modal-menu').innerHTML = selectMenu;
        document.getElementById('modal-stylist').innerHTML = selectStylist;
        document.getElementById('modal-datetime').innerHTML =  updateReservationDisplay(document.querySelector('#start')?.getAttribute('data-select-starttime'));
        document.getElementById('modal-price').innerHTML = selectMenuPrice;
        document.getElementById('modal-time').innerHTML = totalMenuTime;
    }
  }
  if (event.target.classList.contains('reservationButton2')) {
    setTimeout(() => {
      ...
      .then(data => {
        ...
        document.getElementById('menulist-main').innerHTML = '';
        document.getElementById('schedule').innerHTML = '';
        const elements = document.querySelectorAll('.reservationButton1');
        elements.forEach(element => {
          element.remove();
        });
        var reservationBtn = document.getElementById('menu-head').innerHTML = '予約完了しました！';
